...
int ve_open(void) {
	...
	writel(0x00130007, (uint8_t*)regs + VE_CTRL);

	version = readl((uint8_t*)regs + VE_VERSION) >> 16;
	printf("[VE SUNXI] VE version 0x%04x opened.\n", version);
...
void *ve_malloc(int size) {
	...
	for (c = &first_memchunk; c != NULL; c = c->next)
		if(c->virt_addr == NULL && c->size >= size) {
			if (best_chunk == NULL || c->size < best_chunk->size) best_chunk = c;
			if (c->size == size) break;
		}

	if (!best_chunk) return NULL;

	left_size = best_chunk->size - size;
...
uint32_t ve_virt2phys(void *ptr) {
	...
	for (c = &first_memchunk; c != NULL; c = c->next) {
		if (c->virt_addr == NULL) continue;
		if (c->virt_addr == ptr) return c->phys_addr;
		if (ptr > c->virt_addr && ptr < (c->virt_addr + c->size))
